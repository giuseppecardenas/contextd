...
5. `phase_chunk_units` / `phase_gc_chunks` (`indexer/phases_chunks.py`) — derive the retrieval `Chunk` nodes beneath every `File` per `[chunking]` profile (fingerprint-gated; delete → batch upsert → `CONTAINS`/`NEXT_SIBLING` edges → stamp `chunk_fingerprint`), then drop orphaned chunks and those of profiles no longer configured.
6. `phase_cluster_topics` (`indexer/phases_topics.py`, only when `[topics] enabled`) — RAPTOR-style `Topic` tree over the File embeddings.
7. `phase_close` — register the corpus as a `Corpus` node; persist stats, `chunk_count` and the chunking-config fingerprint.
...
7. `phase_chunk_units` / `phase_gc_chunks` — retrieval chunks beneath every `Section` (and beneath `File` for non-markdown files); runs after roll-up so the `section_summary` / `llm` prefix modes have summaries to draw on. See [chunking.md](chunking.md).
8. `phase_cluster_topics` — optional `Topic` tree over the Section embeddings.
9. `phase_close` — register the corpus; persist stats.

...
def test_handle_batch_skips_when_branch_not_allowed(tmp_path: Path) -> None:
    from contextd.daemon import _handle_batch, CorpusDaemonEntry
    ...
    with patch("contextd.daemon.branch_is_allowed", return_value=False):
        with patch("contextd.daemon.run_incremental_file") as mock_rif:
            _handle_batch(
                [tmp_path / "a.md"],
                entry,
...
def test_handle_batch_skips_when_git_busy(tmp_path: Path) -> None:
    ...
    with patch("contextd.daemon.branch_is_allowed", return_value=True):
        with patch("contextd.daemon.is_git_busy", return_value=True):
            with patch("contextd.daemon.run_incremental_file") as mock_rif:
                _handle_batch(
                    [tmp_path / "a.md"],
                    entry,
...
def test_handle_batch_calls_run_incremental_for_changed_files(
    ...
    with patch("contextd.daemon.branch_is_allowed", return_value=True):
        with patch("contextd.daemon.is_git_busy", return_value=False):
            with patch(
...
def test_handle_batch_processes_multiple_files_concurrently(
    ...
    with patch("contextd.daemon.branch_is_allowed", return_value=True):
        with patch("contextd.daemon.is_git_busy", return_value=False):
            with patch("contextd.daemon.run_incremental_file", side_effect=slow_rif):
    ...
    assert elapsed < 0.18  # 4 × 0.05s serial = 0.20s; concurrent ~0.05s
...
def test_handle_batch_logs_and_continues_on_error(tmp_path: Path) -> None:
    ...
    with patch("contextd.daemon.branch_is_allowed", return_value=True):
        with patch("contextd.daemon.is_git_busy", return_value=False):
            with patch("contextd.daemon.run_incremental_file", side_effect=side_effect):
                _handle_batch(
                    files,
                    entry,
                    inference_concurrency=1,
...
def test_write_and_read_pid(tmp_path: Path) -> None:
    from contextd.daemon import _write_pid, _read_pid

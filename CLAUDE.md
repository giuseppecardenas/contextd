...
**As of 2026-04-20 (HEAD `9ffa069`; d6720da..9ffa069 local-only, not yet pushed):**
...
**Cursor:** M7 Task 7.1 (MCP server skeleton — stdio transport).

**Test suite:** 172 unit + 61 integration = 233 collected, 227 executed (6 backend-specific skips for Kuzu-vs-Memgraph semantics). `ruff check`, `ruff format --check`, `mypy --strict`, and the abstraction-invariant grep all clean. Integration suite runs Memgraph via Docker (memgraph:latest v3.x) + Kuzu embedded in `tmp_path`.
...
- **M6 `cli.py` at 341 lines — splitting planned for M7** — suggested layout when `ask` + MCP-proxy commands land: `cli/__init__.py` (group + `init` + `main`), `cli/infra.py` (up/down/status), `cli/corpora.py` (add-corpus, list-corpora, index), `cli/query.py` (ask, logs, costs).

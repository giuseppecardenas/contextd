...
**As of 2026-04-20 (HEAD `80abfde`; 1fc047c..80abfde pushed to origin/main):**
...
**Cursor:** M6 Task 6.1 (Click CLI skeleton — `contextd init` / `contextd status`).

**Test suite:** 148 unit + 61 integration = 209 collected, 203 executed (6 backend-specific skips for Kuzu-vs-Memgraph semantics). `ruff check`, `ruff format --check`, `mypy --strict`, and the abstraction-invariant grep all clean. Integration suite runs Memgraph via Docker (memgraph:latest v3.x) + Kuzu embedded in `tmp_path`.

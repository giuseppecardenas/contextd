...
**As of 2026-04-21 (HEAD `ab62d80`; all pushed to origin/main):**
...
- **M11** (Neo4j backend stand-up + Kuzu excision) — complete 9/9. Neo4j Community 5.x replaces KùzuDB as the second storage backend. `Neo4jBackend` (Bolt via `neo4j>=5.15` driver), Neo4j baseline migration (constraints + vector + FTS indexes), full `upsert_node`/`upsert_edge`/`delete_edges`/`exec_read`/`exec_write`/`vector_search`/`full_text_search`, `Neo4jConfig` + factory wiring, docker-compose `--profile memgraph` / `--profile neo4j` (both bind port 7687, only one runs at a time), integration suite parametrized on memgraph + neo4j, default backend flipped to `"neo4j"` (M11.8). Kuzu fully excised in M11.9: `kuzu.py`, `migrations/kuzu/`, `KuzuConfig`, `IMMUTABLE_AFTER_CREATE_BY_LABEL`, and `kuzu` dep all deleted. `BackendName = Literal["memgraph", "neo4j"]`. Commit range: `57fdb39..4d69272`.

**Cursor:** M10 + M11 complete. Next: M12 (end-to-end tests + CI finalisation — original plan).

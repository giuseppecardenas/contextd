...
**As of 2026-04-21 (HEAD `c9db050`; all pushed to origin/main):**
...
**Cursor:** plan-M0 through plan-M11 complete, plus appendix-M11 (Neo4j). Next: M12 (end-to-end tests + CI finalisation), then M13 (docs, README, publish).
...
**Test suite:** 325 unit + 87 integration (parametrized on both backends) = 412 collected. `ruff check`, `ruff format --check`, `mypy --strict`, and the abstraction-invariant grep all clean. Integration suite runs Memgraph via Docker (memgraph:latest v3.x) + Neo4j Community via Docker (`neo4j:5` image). Integration tests require Docker Desktop; they skip at fixture setup if the Docker daemon is unreachable (expected in dev shells without Docker).

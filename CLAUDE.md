...
**Cursor:** M5 Task 5.1 (debounced change queue).

**Test suite:** 139 unit + 59 integration = 198 collected, 192 executed (6 backend-specific skips for Kuzu-vs-Memgraph semantics). `ruff check`, `ruff format --check`, `mypy --strict` all clean. Integration suite runs Memgraph via Docker (memgraph:latest v3.x) + Kuzu embedded in `tmp_path`.

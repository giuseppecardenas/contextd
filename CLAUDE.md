...
**As of 2026-04-20 (HEAD `b10a02a`; 3e0c513..b10a02a local-only, not yet pushed):**
...
**Cursor:** M9 Task 9.1 (Section-granularity support — heading-level corpus indexing).

**Test suite:** 183 unit + 63 integration = 246 collected, 240 executed (6 backend-specific skips for Kuzu-vs-Memgraph semantics). `ruff check`, `ruff format --check`, `mypy --strict`, and the abstraction-invariant grep all clean. Integration suite runs Memgraph via Docker (memgraph:latest v3.x) + Kuzu embedded in `tmp_path`.
...
- **M8.1 `QueryTranslator.translate(corpus=...)` is a no-op** — plan's `cypher.replace("MATCH", "MATCH ", 1)` just added a trailing space, not a real filter. Replaced with `pass` + TODO pending M9/M10 cross-corpus routing design. CLI `ask --corpus` still accepts the flag; it doesn't do anything yet. Must land a real implementation + test before advertising cross-corpus scoping.
- **M8.1 `_CYPHER_FENCE` regex only matches `` ```cypher...``` `` or `` ```...``` ``** — other language tags (`sql`, `gremlin`, `python`) fall through to keyword-line filter. Acceptable because the prompt instructs a cypher fence, but worth noting.
- **M8.1 keyword-line fallback drops multi-line continuation lines** — `MATCH (n)\n-[:REFERENCES]->(m)\nRETURN n` loses the middle line (doesn't start with a keyword). In practice the LLM wraps in a fence; the fallback only fires on malformed output. Revisit with a real integration test against Gemini output in M9.
